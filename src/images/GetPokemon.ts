...
export async function main(ns: NS): Promise<void> {
    ...
    if (ns.args[0] === "help" || args.help || ns.args.length !== 1) {
        ns.tprintf("Displays an image of a specified Pokémon. Pokémon is specified as an argument by their number in the national Pokédex.");
        ns.tprintf("There is a 1 in 4096 that the Pokémon will be shiny!");
        ns.tprintf(`Usage: run ${ns.getScriptName()} [Pokédex number]`);
        ns.tprintf("Example:");
    ...
    if (!await ns.wget(`https://pokeapi.co/api/v2/pokemon/${ns.args[0]}`, scriptDir + "poke.txt")) {
