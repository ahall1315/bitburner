import { NS, AutocompleteData, ProcessInfo } from "@ns";
import { getRandomInt } from "/lib/utils.js";
import { pServPrefix } from "/lib/const.js";

export async function main(ns: NS): Promise<void> {
    let processInfo: ProcessInfo[] = [];
    let ramOptions: number[] = [];
    let ram: number = -1;
    let cost: number = -1;
    let error: boolean = false;
    const pServMaxRam: number = ns.getPurchasedServerMaxRam();
    const pServLimit: number = ns.getPurchasedServerLimit();
    const HackScript = "/scripts/ts/hgw_all.js";
    const CopyScript = "/scripts/ts/scp.js";
    const ChargeScript = "/scripts/ts/stanek/charge_fragment.js";

    const args = ns.flags([["help", false], ["auto", false], ["charge", false]]);
    if (args.help || (ns.args[0] === undefined && args.auto == false)) {
        ns.tprintf("Upgrades the RAM of a purchased server.");
        ns.tprintf("This script also initially exucutes the hacking script on the servers");
        ns.tprintf("Optonal argument --auto to automatically buy and upgrade servers to their max RAM.");
        ns.tprintf("Optonal argument --charge to charge stanek fragments one tenth of the time.");
    ...
    if (args.auto) {
        let servSlots: number = pServLimit - ns.getPurchasedServers().length;
            ...
            for (let i = 0; i < pServLimit; i++) {
                ...
                if (!error) {
                    if (ns.purchaseServer(target, ram) === "") {
                        ...
                        ns.exec(CopyScript, "home", 1, "/scripts/", target);
                        ns.exec(CopyScript, "home", 1, "/lib/", target);
                        ns.exec(CopyScript, "home", 1, "/control/", target);
                        await ns.sleep(500);// Wait for files to copy

                        if (args.charge) {
                            // Charge fragments one tenth of the time
                            if (getRandomInt(ns, 1, 10) == 1) {
                                ns.exec(ChargeScript, target, 1, "--max");
                            } else {
                                ns.exec(HackScript, target, 1, "--killHack")
                            }
                        } else {
                            ns.exec(HackScript, target, 1, "--killHack")
                        }
    ...
    if (error) {
        ns.tprint("ERROR There was an error in running the script");
    } else {
...
export function autocomplete(data: AutocompleteData, args: string[]): string[] {
    return [...data.servers, "help", "--help", "auto", "--auto"];
}