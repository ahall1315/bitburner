...
export async function main(ns: NS): Promise<void> {
    if (ns.args[0] === "help") {
        ns.tprintf("Buys programs you have enough money for from the darkweb");
    ...
    let money: number = -1;
	let canBuy: string[] = [];
	let printPrograms: string = "";
	let buyTerminal: string = "";
	let cost: number = 0;
	let cantBuyCost: number = 0;
	...
	if (confirm) {
        ...
        if (terminalInput !== null) {
            ...
            (terminalInput as any)[handler].onKeyDown({ key: 'Enter', preventDefault: () => null });
        }
